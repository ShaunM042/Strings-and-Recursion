public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        int dIndex = fragmentIndex + 5;
        String newString = description.substring(dIndex);
        int indexEnding = newString.indexOf(';');
        String nameData = newString.substring(0, indexEnding);

        fragmentIndex = description.indexOf("type:");
        dIndex = fragmentIndex + 5;
        newString = description.substring(dIndex);
        indexEnding = newString.indexOf(';');
        String typeData = newString.substring(0, indexEnding);
        fragmentIndex = description.indexOf("mile/unit:");
        dIndex = fragmentIndex + 10;
        newString = description.substring(dIndex);
        indexEnding = newString.indexOf(';');
        String unitData = newString.substring(0, indexEnding);
        ...
        if (typeData.equals("electric")) {
            ...
            dIndex = fragmentIndex + 8;
            newString = description.substring(dIndex);
            indexEnding = newString.indexOf(';');
            String chargerData = newString.substring(0, indexEnding);
        ...
        Vehicle NV = new Vehicle(nameData, fuel, lease);
     ...
     */
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double powerPrice ){
	    for (Vehicle v: vehicles) {
            Fuel fuel = v.getFuel();
            Lease lease = v.getLease();
            
            int numberofMonths = lease.getLeaseLength();
            double usage = fuel.getUsage();
            double allowance = lease.getMileageAllowance();
            double co2PerUnit;
            double fuelPrice;
            ...
            double co2Emission = computeCO2(numberofMonths, usage, allowance, co2PerUnit);
            v.setCO2Emission(co2Emission);
            double fuelCost = computeFuelCost(numberofMonths, usage, allowance, fuelPrice);
            v.setFuelCost(fuelCost);
            double leaseCost = computeLeaseCost(dueAtSigning, numberofMonths, monthlyCost);

            if (fuel.getType() == 1) {
                v.setTotalCost(fuelCost + leaseCost);
            }
            else {
                double chargerCost = fuel.getCharger();
                v.setTotalCost(chargerCost + fuelCost + leaseCost);
            }  
       }
    }
